...
They are optional and are used only to estimate API cost. With blank values, the application still works and displays estimated cost as unavailable. Only fill them after checking the current per-million-token prices for your selected model on Google's [official pricing page](https://ai.google.dev/gemini-api/docs/pricing). They are prices—not token limits and not values provided by your API key.
...
| `MAX_RUN_COST_USD` | No | Stops tool execution if the configured estimate exceeds this value. |
| `GEMINI_INPUT_USD_PER_MILLION` | No | Current input price per million tokens; leave blank if unknown. |
| `GEMINI_OUTPUT_USD_PER_MILLION` | No | Current output price per million tokens; leave blank if unknown. |
